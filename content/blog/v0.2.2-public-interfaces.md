...
We're excited to announce **Aixgo v0.2.2**, featuring the new public `agent` package that enables library-style integration of Aixgo into existing Go applications. This release also includes streaming reliability improvements for the VertexAI provider.

## Introducing the Public Agent Package

The biggest addition in v0.2.2 is the `agent` package—a standalone, minimal-dependency package that exports core interfaces for building custom agents without requiring the full Aixgo framework.

### Why This Matters

Until now, using Aixgo meant adopting the full framework. While powerful, this approach doesn't fit every use case. Many teams have existing Go services where they want to add agent-based functionality without a major architectural change.

The public agent package solves this by providing:
...
Here's how simple it is to create a custom agent:
...
The package exports three main types:
...
These fixes build on the v0.2.0 production hardening work, ensuring that streaming responses clean up resources correctly under all conditions—normal completion, context cancellation, or error scenarios.
...
We've added comprehensive documentation for the public agent package:
...
- **Additional Public Interfaces**: Expanding exportable components based on community feedback
...
We'd love to hear how you're using the public agent package:
